...
def fig1_matrices(connectomes, labels, networks, nets):
    ...
    Post-harmonization, matrices should show reduced between-site artifacts.
    """
...
def fig2_graph_viz(graphs, networks, nets):
    ...
    fig.suptitle(
        ...
        "Nodes = CC200 ROIs, colored by network | "
        "Top 300 edges shown | Threshold |z| > 0.20 | ComBat harmonized",
...
def fig3_features(graphs):
    ...
    Post-harmonization, any separation reflects biological differences.
    """
    asd_feats  = np.stack([g["x"] for g in graphs if g["y"] == 1])
    ...
    fig.suptitle(
        ...
        "All 200 CC200 regions x all ABIDE subjects | ComBat harmonized",
        fontsize=12, fontweight="bold",
    )
    plt.tight_layout()
...
def fig4_performance(res):
    ...
    plt.tight_layout()
    fig.savefig(OUTDIR / "fig4_performance.png", bbox_inches="tight")
    plt.close()
    print("Fig 4 saved")
...
def fig6_node_importance(res, networks, nets):
    ...
    by functional network. Post-harmonization, patterns reflect biology.
    """
    ...
    cb.set_label("Normalized importance\n(0 = least different, 1 = most different)",
                 fontsize=9)
